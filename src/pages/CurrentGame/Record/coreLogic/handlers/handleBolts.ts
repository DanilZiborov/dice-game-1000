...
export const handleBolts = (player: Player, points: number, status: PlayerStatus, game: Game): Player => {
  ...
  if (player.boltsNumber < MAX_BOLTS - 1) {
    ...
    updated = {
      ...updated,
      boltsNumber: player.boltsNumber + 1,
    };
  } else {
    // иначе - списываем очки и обнуляем болты
    updated = {
      ...updated,
      score: player.score - boltsLimit < 0 ? 0 : player.score - boltsLimit,
      boltsNumber: 0,
    };
  }
